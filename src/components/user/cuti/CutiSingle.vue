<template>
  ...
  <v-layout row wrap>
      ...
      <v-card :color="statusTransaksi(transaksi.status)" dark style="margin-bottom:10px;">
        <v-card-text>
          ...
          <div>
            ...
            <span v-if="transaksi.status == 5"> Mahasiswa Cuti </span>
          </div>
        </v-card-text>
      </v-card>
    ...
    <v-flex xs12 md4 style="margin-right:20px;">
      <v-progress-linear v-bind:indeterminate="loading.cuti" v-if="loading.cuti"></v-progress-linear>
      <v-card v-if="loading.cuti == false">
        <v-card-title>
          <h5 style="margin-bottom:0;">Cuti</h5>
        </v-card-title>
        <v-divider></v-divider>
...
<script>
  ...
  export default {
    ...
    methods: {
      getCuti (id) {
        this.loading.cuti = true
        fb.database().ref('cuti').child(id).on('value', snap => {
          this.cuti = snap.val()
          this.loading.cuti = false
      ...
      init () {
        this.idCuti = this.$route.params.idCuti
      ...
      pembayaranStatusTiga () {
        ...
        let task = storageRef.put(this.formPembayaran.bukti)
        task.on('state_changed', (snap) => {
          ...
          let updates = {}
          updates['/transaksi_cuti/' + this.$route.params.idTransaksi + '/bayar'] = this.formPembayaran.bayar
          updates['/transaksi_cuti/' + this.$route.params.idTransaksi + '/catatanMahasiswa'] = this.formPembayaran.catatanMahasiswa
          updates['/transaksi_cuti/' + this.$route.params.idTransaksi + '/noRekeningMahasiswa'] = this.formPembayaran.noRekeningMahasiswa
          updates['/transaksi_cuti/' + this.$route.params.idTransaksi + '/rekeningId'] = this.formPembayaran.rekening
          updates['/transaksi_cuti/' + this.$route.params.idTransaksi + '/bukti'] = fotoName
          updates['/transaksi_cuti/' + this.$route.params.idTransaksi + '/status'] = 4
      ...
      exportPDF () {
        axios.get(window.location.origin + '/static/img/logoStikom.png', {
          ...
          let data = {
            ...
            alamat: this.detailUser.alamat,
            noHp: this.detailUser.noHp,
